...
# --- End Spark ---"#;
    ...
    if skill_installed {
        println!(
            "  [+] Skill: {}",
            agents_skill_dir.join("SKILL.md").display()
        );
    ...
    if let Some(value) = set {
        ...
        match key.as_str() {
            ...
            "stale_threshold_days" => {
                config.stale_threshold_days = value
                    .parse()
                    .map_err(|_| color_eyre::eyre::eyre!("Invalid number: {}", value))?;
                config.save()?;
                println!("  stale_threshold_days = {}", value);
            }
            "max_scan_depth" => {
                config.max_scan_depth = value
                    .parse()
                    .map_err(|_| color_eyre::eyre::eyre!("Invalid number: {}", value))?;
                config.save()?;
            ...
            "use_trash" => {
                config.use_trash = value
                    .parse()
                    .map_err(|_| color_eyre::eyre::eyre!("Expected true/false: {}", value))?;
                config.save()?;
            ...
            _ => {
                ...
                eprintln!(
                    "  Available: repos_root, stale_threshold_days, max_scan_depth, use_trash"
                );
                std::process::exit(1);
            }
        }
        ...
        println!(
            "  large_artifact_threshold = {} bytes",
            config.large_artifact_threshold
        );
...
pub fn cmd_doctor(config: &config::SparkConfig) {
    ...
    let mut check = |label: &str, ok: bool, fix: &str| {
        if ok {
            ...
            println!("  \x1b[31mx\x1b[0m {}  \x1b[90m-> {}\x1b[0m", label, fix);
            fail += 1;
        }
    };
    ...
    let root_writable = root_exists
        && std::fs::metadata(&config.repos_root)
            .map(|m| !m.permissions().readonly())
            .unwrap_or(false);
    let config_dir = dirs::config_dir().unwrap_or_default().join("spark");
    let root_source = if config_dir.join("config.toml").exists() {
        "configured"
    } else {
        "auto-detected"
    };
    ...
    check(
        "whitelist.txt",
        config_dir.join("whitelist.txt").exists(),
    ...
    let has_spark_cd = std::fs::read_to_string(&rc_file)
        .unwrap_or_default()
        .contains("spark-cd");
    check(
        &format!(
    ...
    let has_completions = if shell.contains("zsh") {
        ...
        home.join(".local/share/bash-completion/completions/spark")
            .exists()
    } else {
        false
    };
    ...
    if cfg!(target_os = "macos") {
        let lsof_ok = std::process::Command::new("lsof")
            .arg("-v")
            .output()
            .map(|_| true)
            .unwrap_or(false);
