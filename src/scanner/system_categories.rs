...
fn scan_docker_category(
    ...
    Some(CleanableItem {
        ...
        size: parse_docker_sizes(&stdout),
...
pub fn scan_caches(whitelist: &[PathBuf]) -> Vec<CleanableItem> {
    ...
    for t in &targets {
        if !t.path.exists() || is_whitelisted(&t.path, whitelist) {
            continue;
        }
        ...
        let app_running = if t.app.is_empty() {
            false
        } else {
            is_app_running(t.app)
        };
        let clean_cmd = if !t.cmd.is_empty() && !t.args.is_empty() {
            ...
            if exists {
                CleanCommand::Shell(
                    t.cmd.to_string(),
                    t.args.iter().map(|s| s.to_string()).collect(),
                )
            } else {
                CleanCommand::RemoveDir(t.path.clone())
            }
        } else {
            CleanCommand::RemoveDir(t.path.clone())
        };
        ...
        let detail = {
            let p = t.path.display().to_string();
        ...
        let warn = if app_running {
            format!(" ({} running)", t.app)
        } else {
            String::new()
        };
    ...
    for dir in &log_dirs {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if let Some(days) = age_days {
                if days < super::system_cleaner::LOG_AGE_DAYS {
                    continue;
                }
            }
            let name = path
...
pub fn scan_downloads(whitelist: &[PathBuf]) -> Vec<CleanableItem> {
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        let path = entry.path();
        if !path.is_file() || is_whitelisted(&path, whitelist) {
            continue;
        }
        let ext = path
            .extension()
